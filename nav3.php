<div class="relative sticky top-0 z-50 transition duration-700 ease-in delay-1000" x-data="scrollToReveal()" x-ref="navbar" x-on:scroll.window="scroll()" x-bind:class="{
                   'sticky top-0': sticky,
                   'relative': !sticky
                   }"
                   >

  <nav class="relative bg-indigo-900" x-data="{ open: false }">
    <div class="px-2 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="relative flex items-center justify-between h-16">
        <div class="absolute inset-y-0 right-0 flex items-center md:hidden">
          <!-- Mobile menu button-->
          <button x-on:click="open = true " class="inline-flex items-center justify-center p-2 text-yellow-400 rounded-md hover:text-yellow-400 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-expanded="false">
    ...
    <div class="relative md:hidden" x-show="open" x-on:click.away="open = false" x-init="() => { $el.classList.remove('hidden');$el.classList.add('block') }"
    >
